...
#            after StandardScaler).  σ² from SVD are on a completely
...
#            ronne raised from pipeline_config's 0.7× to 2× so they
#            aren't actively downweighted.
#
#  FIX 7  — Amundsen override threshold raised 0.25 → 0.35.
#            Amundsen TPR=0.93 in run 1 (✅ PASS, target 0.50).  Problem
...
def plot_eigenvalues(s_array, explained_ratio, model_name="SVD"):
    ...
    Cumulative panel y-axis capped at 105 — will now show correct % values.
    """
    ...
    ax.bar(x[:len(s_array)], s_array, color="#3498db", alpha=0.8)
    ax.set_xlabel("Component", color="white")
    ax.set_ylabel("Singular Value (σ)", color="white")
    ax.set_title("Singular Value Decay\n(RowMatrix.computeSVD)", color="white")
    ax.tick_params(colors="white")

    ax = axes[1]
    ax.set_facecolor(dark)
    ax.bar(x, explained_ratio * 100, color="#2ecc71", alpha=0.8)
    ax.set_xlabel("Component", color="white")
    ax.set_ylabel("Explained Variance (%)", color="white")
    ax.set_title("Per-Component Variance\n(PCA model)", color="white")
    ax.tick_params(colors="white")

    ax = axes[2]
    ax.set_facecolor(dark)
    ax.plot(x, cumulative * 100, "o-", color="#e74c3c", linewidth=2, markersize=4)
    ax.axhline(y=90, color="#e67e22", linestyle="--", alpha=0.7, label="90% threshold")
    ax.set_ylim(0, 105)
    ax.set_xlabel("Number of Components", color="white")
    ax.set_ylabel("Cumulative Variance (%)", color="white")
    ...
    fig.suptitle(f"{model_name}: Eigenvalue Analysis", color="white", fontsize=14)
    out = os.path.join(OUTPUT_DIR, f"{model_name}_eigenvalue_analysis.png")
    fig.savefig(out, dpi=150, bbox_inches="tight", facecolor=fig.get_facecolor())
    plt.close(fig)
    print(f"  [{model_name}] Eigenvalue plot → {out}")
...
def plot_clusters(df_clustered, k_eff, model_name="SVD_KMeans"):
    ...
    pal = ["#e74c3c", "#3498db", "#2ecc71", "#9b59b6",
           "#e67e22", "#1abc9c", "#f1c40f", "#e91e63"]

    fig, ax = plt.subplots(figsize=(10, 8))
    fig.patch.set_facecolor("#2d2d2d"); ax.set_facecolor("#1e1e1e")
    for cid in sorted(pdf["cluster"].unique()):
        sub = pdf[pdf["cluster"] == cid]
        ax.scatter(sub["svd_0"], sub["svd_1"], c=pal[cid % len(pal)],
                   label=f"Cluster {cid}", alpha=0.5, s=3)
    ax.set_xlabel("SVD Component 1", color="white")
    ax.set_ylabel("SVD Component 2", color="white")
    ax.set_title(f"{model_name}: Clusters in PC Space", color="white")
    ax.tick_params(colors="white")
    ax.legend(facecolor="#3d3d3d", edgecolor="white", labelcolor="white", markerscale=3)
    p1 = os.path.join(OUTPUT_DIR, f"{model_name}_cluster_scatter.png")
    fig.savefig(p1, dpi=150, bbox_inches="tight", facecolor=fig.get_facecolor())
    plt.close(fig)
    print(f"  [{model_name}] Cluster scatter → {p1}")

    if LABEL_COL in pdf.columns:
        fig2, ax2 = plt.subplots(figsize=(10, 8))
        fig2.patch.set_facecolor("#2d2d2d"); ax2.set_facecolor("#1e1e1e")
        lc = {0: "#95a5a6", 1: "#e74c3c"}
        for lv in [0, 1]:
            sub = pdf[pdf[LABEL_COL] == lv]
            ax2.scatter(sub["svd_0"], sub["svd_1"], c=lc[lv], label=f"Label={lv}",
                        alpha=0.5 if lv == 0 else 0.8, s=2 if lv == 0 else 6)
        ax2.set_xlabel("SVD Component 1", color="white")
        ax2.set_ylabel("SVD Component 2", color="white")
        ax2.set_title(f"{model_name}: Ground Truth in PC Space", color="white")
        ax2.tick_params(colors="white")
        ax2.legend(facecolor="#3d3d3d", edgecolor="white", labelcolor="white", markerscale=3)
        p2 = os.path.join(OUTPUT_DIR, f"{model_name}_label_scatter.png")
        fig2.savefig(p2, dpi=150, bbox_inches="tight", facecolor=fig2.get_facecolor())
        plt.close(fig2)
        print(f"  [{model_name}] Label scatter → {p2}")
...
def regional_prauc_breakdown(preds_df, model_name):
    ...
    if amundsen_tpr is not None:
        status = "✅ PASS" if amundsen_tpr >= 0.50 else "❌ FAIL (target ≥ 50%)"
