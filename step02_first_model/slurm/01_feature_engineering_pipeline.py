...
def build_label(df):
    """
    Revised label construction — absolute thresholds replacing
    mascon-relative percentile ranking.

    Changes from original:
    - GRACE flag: absolute anomaly from mascon long-run mean (not p25)
    - ICESat flag: anomaly from pixel mean relative to REGIONAL std
      (not pixel-level std, which adapts to sustained thinning)
    - Both thresholds calibrated per-region to account for baseline
    ...
    # ── GRACE flag: lwe_fused < mascon long-run mean - 0.5*mascon_std ──
    # Uses long-run mascon stats instead of monthly p25.
    # This fires when a pixel's mass anomaly is genuinely anomalous
    # relative to the mascon's multi-year baseline — not just below
    # the median of recent months.
    mascon_stats = (
        df.groupBy("mascon_id")
        .agg(
            F.avg("lwe_quarterly_avg").alias("_mascon_lwe_mean"),
            F.stddev("lwe_quarterly_avg").alias("_mascon_lwe_std"),
        )
    )
    df = df.join(F.broadcast(mascon_stats), on="mascon_id", how="left")
    grace_flag = F.coalesce(
        F.col("lwe_quarterly_avg") < (
            F.col("_mascon_lwe_mean") - F.lit(0.5) * F.col("_mascon_lwe_std")
        ),
        F.lit(False),
    )
    ...
    if LABEL_MODE == "grace_anomaly":
        ...
        # ── ICESat flag: delta_h < regional mean - 0.5*regional std ────
        # Uses regional (not pixel-level) std as the denominator.
        # Pixel-level std adapts to sustained thinning and becomes too
        # permissive in basins like Amundsen Sea with multi-year trends.
        # Regional std is stable and provides a continent-meaningful scale.
        region_stats = (
            df.groupBy("regional_subset_id", "month_idx")
            .agg(
                F.avg("delta_h").alias("_reg_dh_mean"),
                F.stddev("delta_h").alias("_reg_dh_std"),
            )
        )
        df = df.join(
            F.broadcast(region_stats),
            on=["regional_subset_id", "month_idx"],
            how="left",
        )
        icesat_flag = F.coalesce(
            F.col("delta_h") < (
                F.col("_reg_dh_mean") - F.lit(0.5) * F.col("_reg_dh_std")
            ),
            F.lit(False),
        )
        df = df.withColumn(LABEL_COL, (grace_flag & icesat_flag).cast(IntegerType()))
        df = df.drop("_reg_dh_mean", "_reg_dh_std")

    # LEAKAGE FIREWALL: ensure targets are not used as features
    df = df.drop(
        "_mascon_lwe_mean", "_mascon_lwe_std",
        "lwe_fused",
    )
    assert "lwe_fused" not in df.columns, "LEAKAGE BUG: lwe_fused survived!"
    print(f"[label] Revised dual-sensor label (label_mode={LABEL_MODE!r}). "
          f"lwe_fused confirmed absent.")
